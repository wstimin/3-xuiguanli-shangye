...
本系统基于 3-xui 面板 3.4.1 版本开发和测试。其他 3-xui 版本通常也可以接入，但不同版本或魔改版可能存在 API 差异，建议部署后先在“3x-ui 节点”里点击测试，确认入站列表、用户同步、SOCKS 路由写入都正常。
...
```bash
PORT=8080 REPO_URL=https://github.com/wstimin/3-xuiguanlimianban.git bash <(curl -fsSL https://raw.githubusercontent.com/wstimin/3-xuiguanlimianban/main/install.sh)
```

自定义安装目录：

```bash
APP_DIR=/opt/my-shiye-panel REPO_URL=https://github.com/wstimin/3-xuiguanlimianban.git bash <(curl -fsSL https://raw.githubusercontent.com/wstimin/3-xuiguanlimianban/main/install.sh)
```

指定 GitHub 仓库：

```bash
REPO_URL=https://github.com/wstimin/3-xuiguanlimianban.git bash install.sh
```
...
```bash
cd /opt
git clone https://github.com/wstimin/3-xuiguanlimianban.git shiye-management-system
